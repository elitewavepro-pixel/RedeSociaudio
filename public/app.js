...
async function renderKnowledge(){let items=await api('/api/knowledge');let featured=items.filter(x=>x.is_featured),cats=[...new Set(items.map(x=>x.category))];content.innerHTML=`<section class="knowledge-hero knowledge-hero-premium card"><div class="knowledge-hero-copy"><span class="eyebrow">BIBLIOTECA TÉCNICA VIVA</span><h1>Centro de Conhecimento</h1><p>Guias, artigos e soluções práticas de áudio profissional, organizados para não se perderem no feed.</p><div class="knowledge-hero-actions"><button class="knowledge-publish-btn" onclick="openArticleDialog()">${icon('plus')} Publicar artigo</button><span class="knowledge-hero-note">${icon('book')} Conteúdo técnico para profissionais de áudio</span></div></div><div class="knowledge-hero-visual"><div class="knowledge-hero-ring ring-a"></div><div class="knowledge-hero-ring ring-b"></div><div class="knowledge-hero-icon">${icon('book')}</div><span>APRENDA • COMPARTILHE • EVOLUA</span></div></section><div class="knowledge-toolbar"><input id="knowledgeSearch" placeholder="Pesquisar microfonia, Ui24R, acústica..."><select id="knowledgeCategory"><option value="">Todas as categorias</option>${cats.map(c=>`<option>${esc(c)}</option>`).join('')}</select><select id="knowledgeLevel"><option value="">Todos os níveis</option><option>Iniciante</option><option>Intermediário</option><option>Avançado</option></select></div>${featured.length?`<section><div class="section-heading"><div><h2>Conteúdos em destaque</h2><p>Materiais selecionados pela comunidade.</p></div></div><div class="knowledge-grid featured-grid">${featured.slice(0,3).map(articleCard).join('')}</div></section>`:''}<section><div class="section-heading"><div><h2>Biblioteca completa</h2><p>${items.length} conteúdo(s) disponível(is).</p></div></div><div id="knowledgeGrid" class="knowledge-grid"></div></section>`;let draw=()=>{let q=knowledgeSearch.value.toLowerCase(),cat=knowledgeCategory.value,lvl=knowledgeLevel.value;let filtered=items.filter(a=>(!q||[a.title,a.summary,a.body,a.category,a.author_name].join(' ').toLowerCase().includes(q))&&(!cat||a.category===cat)&&(!lvl||a.difficulty===lvl));knowledgeGrid.innerHTML=filtered.length?filtered.map(articleCard).join(''):'<div class="card empty">Nenhum conteúdo encontrado.</div>';hydrateIcons(knowledgeGrid)};knowledgeSearch.oninput=draw;knowledgeCategory.onchange=draw;knowledgeLevel.onchange=draw;draw();hydrateIcons(content)}
